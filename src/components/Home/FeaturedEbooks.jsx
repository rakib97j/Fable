  ...
  return (
    <section className="w-full py-12 md:py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto font-sans">
      ...
      >
        ...
        >
          View all
          <ArrowRight className="w-4 h-4 transition-transform duration-200 group-hover:translate-x-1 text-rose-400" />
        </Link>
      </motion.div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((n) => (
            <div
              key={n}
              className="bg-[#121215] border border-zinc-800/60 p-4 space-y-4 animate-pulse rounded-none"
            >
              <div className="aspect-3/4 bg-zinc-800/50 w-full rounded-none" />
              <div className="h-4 bg-zinc-800/60 rounded-none w-3/4" />
              <div className="h-3 bg-zinc-800/40 rounded-none w-1/2" />
              <div className="h-4 bg-zinc-800/60 rounded-none w-1/3 pt-2" />
            </div>
          ))}
        </div>
      ) : ebooks.length === 0 ? (
        /* Empty State */
        <div className="text-center py-12 border border-zinc-800/60 bg-[#121216]/50 rounded-none">
            ...
            return (
              ...
              >
                <Link
                  ...
                  className="group relative bg-[#121215] border border-zinc-800/80 hover:border-rose-500/40 shadow-lg overflow-hidden transition-colors duration-200 flex flex-col justify-between h-full rounded-none block"
                >
                  ...
                  <div>
                    <div className="relative aspect-3/4 w-full overflow-hidden bg-zinc-900">
                      ...
                      {/* Genre Badge */}
                      {ebook.genre && (
                        <span className="absolute top-3 left-3 bg-zinc-900/90 backdrop-blur-md border border-zinc-700/60 text-zinc-300 text-[11px] font-medium px-2.5 py-1 uppercase tracking-wider rounded-none">
                          {ebook.genre}
                        </span>
                      )}
